...
// Production-friendly error reporting (important errors only).
// display_errors is turned on only when esoTalk.debug is enabled (after config load).
ini_set("display_errors", "0");
error_reporting(E_ERROR | E_PARSE | E_CORE_ERROR | E_COMPILE_ERROR | E_USER_ERROR | E_RECOVERABLE_ERROR);

// Ensure a default timezone is always set (UTC is a safe modern default).
if (ini_get("date.timezone") == "") {
	date_default_timezone_set("UTC");
}

// Basic hardening: do not expose PHP version in headers when possible.
@ini_set("expose_php", "0");

// Modern security headers (safe defaults; adjust at web-server level if needed)
if (!headers_sent()) {
	header("X-Content-Type-Options: nosniff");
	header("X-Frame-Options: SAMEORIGIN");
	header("Referrer-Policy: strict-origin-when-cross-origin");
	header("X-XSS-Protection: 0"); // modern browsers use CSP; legacy header can cause issues
...
// Determine the relative path to this forum (e.g. /esoTalk).
// Robust for Apache rewrite, XAMPP/Windows, nginx, and subfolder installs.
...
// Fallback: derive from REQUEST_URI when SCRIPT_NAME is unreliable
if (ET::$webPath === "" && !empty($_SERVER["REQUEST_URI"])) {
	$uri = $_SERVER["REQUEST_URI"];
	if (($q = strpos($uri, "?")) !== false) $uri = substr($uri, 0, $q);
	$uri = str_replace("\\", "/", $uri);
	if (preg_match('#^(/.+?)/index\.php#', $uri, $m)) {
		ET::$webPath = $m[1];
	} elseif (preg_match('#^(/[^/]+)/index\.php#i', $uri, $m)) {
		ET::$webPath = $m[1];
	}
}
// Normalize: never leave a trailing slash on webPath (except empty)
ET::$webPath = rtrim(ET::$webPath, "/");
if (ET::$webPath === "/") ET::$webPath = "";


// Undo register_globals (legacy, no longer present on modern PHP).
undoRegisterGlobals();

// Magic quotes removed in PHP 7.4+; this block is a no-op on modern runtimes.
// get_magic_quotes_gpc() removed in PHP 7.4 — no stripslashes needed.
...
ETFactory::register("cacheAPCu", "ETCacheAPCu", PATH_LIBRARY."/ETCacheAPCu.class.php");
...
// If esoTalk hasn't been installed, register only the install controller.
...
// Installed: always register normal controllers so the site never goes fully 404
// just because the version string changed.
else {
	...
	if (file_exists(PATH_CONTROLLERS."/ETPostController.class.php"))
	...
	ETFactory::registerController("sitemap", "ETSitemapController", PATH_CONTROLLERS."/ETSitemapController.class.php");
	...
	ETFactory::registerAdminController("languages", "ETLanguagesAdminController", PATH_CONTROLLERS."/admin/ETLanguagesAdminController.class.php");
		if (in_array(C("esoTalk.registration.requireConfirmation"), array("approval", "email+approval"), true))
	...
	// Version mismatch: register upgrade and make it the default landing page.
	if (C("esoTalk.version") != ESOTALK_VERSION) {
		ETFactory::registerController("upgrade", "ETUpgradeController", PATH_CONTROLLERS."/ETUpgradeController.class.php");
		ET::$config["esoTalk.defaultRoute"] = "upgrade";
	}
}
...
// "file" / true / "cache" → ETCache (disk under cache/data/)
// "cacheAPCu" / "apcu" → APCu when extension exists
// false / "" → still ETCache (harmless in-memory+disk)
$cacheClass = C("esoTalk.cache");
if ($cacheClass === "APCu" || $cacheClass === "apcu" || $cacheClass === "cacheAPCu") {
	ET::$cache = ETFactory::make("cacheAPCu");
} else {
	// Default file cache (ETCache already implements disk storage)
	ET::$cache = ETFactory::make("cache");
}

// Connect to the database.
ET::$database = ETFactory::make("database");
ET::$database->init(C("esoTalk.database.host"), C("esoTalk.database.user"), C("esoTalk.database.password"), C("esoTalk.database.dbName"), C("esoTalk.database.prefix"), C("esoTalk.database.connectionOptions"), C("esoTalk.database.port"), C("esoTalk.database.driver", "sqlite"), C("esoTalk.database.sqlitePath", PATH_ROOT . "/config/esotalk.sqlite"));
...
// Resolve the page request path.
$request = "";

// 1) Preferred: ?p=controller/method (works without rewrite)
if (!empty($_GET["p"])) {
	$request = (string) $_GET["p"];
	unset($_GET["p"]);
}
// 2) PATH_INFO (Apache index.php/foo or AcceptPathInfo)
elseif (!empty($_SERVER["PATH_INFO"]) && $_SERVER["PATH_INFO"] !== "/") {
	$request = trim((string) $_SERVER["PATH_INFO"], "/");
}
// 3) ORIG_PATH_INFO (some IIS/XAMPP setups)
elseif (!empty($_SERVER["ORIG_PATH_INFO"]) && $_SERVER["ORIG_PATH_INFO"] !== "/") {
	$request = trim((string) $_SERVER["ORIG_PATH_INFO"], "/");
}
// 4) REQUEST_URI minus webPath
elseif (isset($_SERVER["REQUEST_URI"])) {
	...
	// Strip leading /index.php or index.php/
	$uri = preg_replace('#^/?index\.php(/|$)#i', "/", $uri);
	$request = trim(urldecode($uri), "/");
	if (strcasecmp($request, "index.php") === 0) $request = "";
}

// Never treat the forum folder name itself as a controller
if ($request !== "" && ET::$webPath !== "") {
	$folder = trim(ET::$webPath, "/");
	$folder = rawurldecode($folder);
	if (strcasecmp($request, $folder) === 0) {
		$request = "";
	} elseif (stripos($request, $folder."/") === 0) {
		$request = substr($request, strlen($folder) + 1);
	}
}

// Clean accidental double segments
...
$request = preg_replace('/[\x00-\x1F\x7F]/', '', $request);
