...
public function action_index($conversationId = false, $year = false, $month = false)
{
	...
	if ($this->responseType === RESPONSE_TYPE_DEFAULT) {
		...
		// Add one meta description tag. A configured classic forum description takes priority;
		// otherwise use an excerpt from the first post's content.
		$description = C("esoTalk.metaDescription", false) ? trim((string)C("esoTalk.forumDescription", "")) : "";
		if (!$description && $conversation["countPosts"] > 0) {
			$description = ET::SQL()
		...
		if ($description) $this->addToHead("<meta name='description' content='".sanitizeHTML($description)."'>");
...
public function action_start($member = false)
{
	...
	$channels = ET::channelModel()->getByPermission("start");
	$channelId = $form->validPostBack("content") ? ET::$session->getValue("channelId") : ET::$session->getValue("searchChannelId");
	ET::$session->store("channelId", isset($channels[$channelId]) ? $channelId : array_key_first($channels));
	...
	if ($form->validPostBack("content")) {
		...
		$result = $model->createConversation(array(
			"title" => $form->getValue("title"),
			"channelId" => ET::$session->getValue("channelId"),
			"content" => $form->getValue("content"),
		), ET::$session->getValue("membersAllowed"), $form->isPostBack("saveDraft"));
...
public function action_changeChannel($conversationId = "")
{
	...
	$channels = ET::channelModel()->getByPermission();
	...
	if ($conversation["conversationId"]) {
		...
		if ($channelId = $form->getValue("channel"))
			...
			if ($model->errorCount())
				...
				// Fetch the new conversation details before any redirect.
				$conversation = $model->getById($conversation["conversationId"]);
				// Allow addons to persist additional conversation fields before redirecting.
				$this->trigger("conversationSaveAfter", array(&$conversation, $form));
...
protected function formatPostForTemplate($post, $conversation)
{
	...
	$formatted = array(
		...
		"info" => array(),
		"controls" => array(),
		"body" => !empty($post["deleteMemberId"]) ? false : (string)$this->displayPost(isset($post["content"]) ? $post["content"] : ""),
		"footer" => array(),
	...
	$formatted["info"][] = "<a href='".URL(postURL($post["postId"]))."' class='time' title='".esotalk_strftime(T("date.full"), $post["time"])."'>".(!empty($conversation["searching"]) ? T("Show in context") : $date)."</a>";
	...
	if (!$post["deleteMemberId"]) {
		...
		if (empty($post["preferences"]["hideOnline"])) {
				...
				if (is_array($lastAction)) {
					$lastActionText = !empty($lastAction[0]) ? " (".sanitizeHTML($lastAction[0]).")" : "";
					$lastActionURL = !empty($lastAction[1]) ? $lastAction[1] : "";
					array_unshift($formatted["info"], "<".($lastActionURL ? "a href='".sanitizeHTML($lastActionURL)."'" : "span")." class='online' title='".T("Online").$lastActionText."'><i class='icon-circle'></i></".($lastActionURL ? "a" : "span").">");
				}
		}
		...
		if ($post["editMemberId"]) $formatted["controls"][] = "<span class='editedBy'>".sprintf(T("Edited %s by %s"), "<span title='".esotalk_strftime(T("date.full"), $post["editTime"])."'>".relativeTime($post["editTime"], true)."</span>", name($post["editMemberName"]))."</span>";
	...
	// But if the post IS deleted...
		...
		if ($post["deleteMemberId"]) $formatted["controls"][] = "<span>".sprintf(T("Deleted %s by %s"), "<span title='".esotalk_strftime(T("date.full"), $post["deleteTime"])."'>".relativeTime($post["deleteTime"], true)."</span>", name($post["deleteMemberName"]))."</span>";
...
protected function displayPost($content)
{
	...
	$words = ET::$session ? ET::$session->getValue("highlight") : null;
	try {
		$out = ET::formatter()->init($content)->highlight($words)->format()->get();
		if (!is_string($out)) $out = "";
		// If formatting wiped the text but original had content, fall back to plain text
		if (trim(strip_tags($out)) === "" && trim($content) !== "") {
			$out = "<p>".nl2br(sanitizeHTML($content))."</p>";
		}
		return $out;
	} catch (Throwable $e) {
		return "<p>".nl2br(sanitizeHTML($content))."</p>";
	}
}
