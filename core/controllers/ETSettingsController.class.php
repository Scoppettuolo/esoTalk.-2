...
public function action_general()
{
	...
	// Add the avatar section and field to the form.
		...
		$avatarFormat = (string)ET::$session->user["avatarFormat"];
		$form->setValue("avatarSource", !$avatarFormat ? "none" : (preg_match("~^https?://~i", $avatarFormat) ? "url" : "upload"));
		$form->setValue("avatarURL", preg_match("~^https?://~i", $avatarFormat) ? $avatarFormat : "");
...
public function fieldAvatar($form)
{
	return "<div class='avatarChooser'>".
		...
		"<div class='avatarSources'>".
		"<label class='radio'>".$form->radio("avatarSource", "upload")." ".T("Upload an avatar")."<br/>".$form->input("avatar", "file")."</label>".
		"<label class='radio'>".$form->radio("avatarSource", "url")." ".T("Use an avatar URL")."<br/>".$form->input("avatarURL", "text", array("class" => "text", "placeholder" => "https://"))."</label>".
		"<label class='radio'>".$form->radio("avatarSource", "none")." ".T("Use no avatar")."</label>".
		"</div>".
...
{
	$current = (string)ET::$session->user["avatarFormat"];
	$source = $form->getValue("avatarSource");

	if ($source === "none") {
		$this->removeLocalAvatar($current);
		ET::memberModel()->updateById(ET::$session->userId, array("avatarFormat" => null));
		return;
	}

	if ($source === "url") {
		$url = trim((string)$form->getValue("avatarURL"));
		if (!preg_match("~^https?://[^\\s]+$~i", $url)) {
			$form->error("avatarURL", T("message.invalidAvatarURL"));
			return;
		}
		$this->removeLocalAvatar($current);
		ET::memberModel()->updateById(ET::$session->userId, array("avatarFormat" => $url));
		return;
	}

	// An unchanged upload radio with no new file keeps the current avatar.
	if (empty($_FILES[$key]["tmp_name"])) return;

	$uploader = ET::uploader();
	try {
		$file = $uploader->getUploadedFile($key, array("image/jpeg", "image/png", "image/gif", "image/pjpeg", "image/x-png"));
...
protected function removeLocalAvatar($format)
{
	if (!$format || preg_match("~^https?://~i", (string)$format)) return;
	$format = basename((string)$format);
	if (preg_match("~^[a-z0-9]+$~i", $format)) @unlink(PATH_UPLOADS."/avatars/".ET::$session->userId.".".$format);
}
