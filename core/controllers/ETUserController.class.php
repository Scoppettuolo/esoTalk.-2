...
public function action_join()
{
	...
	if ($form->validPostBack("submit")) {
		...
		if (!$form->errorCount()) {
				...
				$confirmation = C("esoTalk.registration.requireConfirmation");
				if ($confirmation === "email+approval") $data["account"] = ACCOUNT_PENDING;

				if (!$confirmation) $data["confirmed"] = true;
				else $data["resetPassword"] = generateSecureToken(16);
			...
			if ($model->errorCount()) $form->errors($model->errors());
					...
					// Combined mode requires both email confirmation and administrator approval.
					elseif ($confirmation === "email+approval") {
						$this->sendConfirmationEmail($data["email"], $data["username"], $memberId.$data["resetPassword"]);
						$admin = ET::memberModel()->getById(C("esoTalk.rootAdmin"));
						ET::activityModel()->createActivity("unapproved", $admin, null, array("username" => $data["username"]));
						$this->renderMessage(T("Success!"), T("message.confirmEmail")." ".T("message.waitForApproval"));
					}

					// If we require the user account to be approved by an administrator, show a message.
					elseif ($confirmation === "approval") {
...
public function action_confirm($hash = "")
{
	...
	$confirmation = C("esoTalk.registration.requireConfirmation");
	if ($confirmation !== "email" && $confirmation !== "email+approval") return;
	...
	if ($result->numRows()) {
		...
		// In combined mode, email confirmation is not enough to log in: approval is still required.
		if ($confirmation === "email+approval")
			$this->message(T("message.emailConfirmed")." ".T("message.waitForApproval"), "success");
		else {
...
public function action_sendConfirmation($username = "")
{
	...
	$tmp = ET::memberModel()->get(array("m.username" => $username, "m.confirmed" => false)); $member = $tmp ? reset($tmp) : null;
	if ($member) {
...
public function action_forgot()
{
	...
	if ($form->validPostBack("submit")) {
		...
		$tmp = ET::memberModel()->get(array("email" => $form->getValue("email"))); $member = $tmp ? reset($tmp) : null;
		if (!$member)
...
public function action_reset($hashString = "")
{
	...
	$tmp = ET::memberModel()->get(array("m.memberId" => $memberId, "resetPassword" => $hash)); $member = $tmp ? reset($tmp) : null;
	if (!$member) return;
