...
init: function() {
	...
	// News feed removed (esoTalk.org offline). Update notification is rendered server-side.
...
init: function() {
	...
	var updateLogoFields = function() {
		var image = $("#adminSettings input[name=forumHeader]:checked").val() == "image";
		var source = $("#adminSettings input[name=forumLogoSource]:checked").val();
		$("#forumLogoOptions").toggle(image);
		$("#adminSettings input[name=forumHeaderImage]").closest("label").toggle(image && source == "upload");
		$("#adminSettings input[name=forumLogoURL]").closest("label").toggle(image && source == "url");
	};
	$("#adminSettings input[name=forumHeader], #adminSettings input[name=forumLogoSource]").change(updateLogoFields);
	updateLogoFields();

	var updateIconFields = function() {
		var iconSource = $("#adminSettings input[name=forumIconSource]:checked").val();
		var faviconSource = $("#adminSettings input[name=shortcutIconSource]:checked").val();
		$("#adminSettings input[name=forumIconImage]").closest("label").toggle(iconSource == "upload");
		$("#adminSettings input[name=forumIconURL]").closest("label").toggle(iconSource == "url");
		$("#adminSettings input[name=shortcutIconImage]").closest("label").toggle(faviconSource == "upload");
		$("#adminSettings input[name=shortcutIconURL]").closest("label").toggle(faviconSource == "url");
	};
	$("#adminSettings input[name=forumIconSource], #adminSettings input[name=shortcutIconSource]").change(updateIconFields);
	updateIconFields();
