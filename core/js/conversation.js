...
init: function() {
	...
	$(window).on("beforeunload.conversation", ETConversation.beforeUnload);
},
...
initReply: function() {
	...
	// Keep submit controls in sync with the actual textarea value. The old keyup-only
	// handler left the button disabled after paste, autofill, mobile input, or IME input.
	function updateReplyButtons() {
		var hasContent = $.trim(textarea.val() || "").length > 0;
		$("#reply .postReply, #reply .saveDraft")[hasContent ? "enable" : "disable"]();
		ETConversation.editingReply = hasContent;
	}
	updateReplyButtons();

	// React to every browser input path, including paste, autocomplete and mobile keyboards.
	textarea.on("input keyup change", updateReplyButtons);
	textarea.on("paste", function() { setTimeout(updateReplyButtons, 0); });
...
startConversation: function(draft) {
	...
	$.ETAjax({
		...
		success: function(data) {
			// The controller serializes messages as an array. An empty array is truthy
			// in JavaScript, so check its length before keeping the form open.
			var messages = data.messages;
			var hasMessages = messages && (typeof messages.length === "number" ? messages.length > 0 : !$.isEmptyObject(messages));
			if (hasMessages) {
				if (messages.title) $("#conversationTitle input").focus();
				if (messages.content) $("#reply textarea").focus();
				return;
			}

			// A successful AJAX start returns `redirect` (older integrations may use
			// `url`). Navigate after the database insert instead of leaving the user on
			// the form with its controls apparently disabled.
			var redirect = data.redirect || data.url;
			if (redirect) {
				ETConversation.editingReply = false;
...
discardDraft: function() {
	...
	$(window).off("beforeunload.conversation");
...
initMembersAllowed: function() {
	...
	$(document).off("click", "#membersAllowedSheet .allowedList .name a").on("click", "#membersAllowedSheet .allowedList .name a", function(e) {
		e.preventDefault();
