...
{
	// Remote check disabled: esotalk.org is offline. Always report "up to date".
	return false;
}
...
{
	/* XAMPP/Apache may keep config.php bytecode after an atomic replacement.
	 * Invalidate that single file so settings changed in the previous request are
	 * read before any plugin or controller can write configuration again. */
...
public static function writeConfig($values)
{
	...
	/* Build from the configuration already loaded in this request. Re-including the
	 * file after an atomic rename can return stale OPcache bytecode on XAMPP/Apache,
	 * causing successive writes to lose earlier settings (including the active skin). */
	$config = is_array(self::$config) ? self::$config : array();
	$config = array_merge($config, $values);
	self::$config = $config;

	$who = "system";
	if (isset(ET::$session) && ET::$session && !empty(ET::$session->user["username"])) {
		$who = ET::$session->user["username"]." (".(ET::$session->ip ?: "").")";
	}

	$lines = array("<?php");
	foreach ($config as $k => $v) {
		$lines[] = "\$config[".var_export((string)$k, true)."] = ".var_export($v, true).";";
	}
	$lines[] = "";
	$lines[] = "// Last updated by: ".$who." @ ".date("c");
	$contents = implode("
", $lines)."
";
