...
public function __construct()
{
	...
	$messages = ET::$session->getValue("messages");
...
public function init()
{
	...
	if ($this->responseType === RESPONSE_TYPE_DEFAULT) {
		...
		$this->addToMenu("meta", "copyright", "<a href='https://github.com/Scoppettuolo' target='_blank' rel='noopener noreferrer'>".T("Powered by")." esoTalk</a>");

		// Set up some default JavaScript files and language definitions.
		$this->addJSFile("core/js/lib/jquery.min.js", true);
		$this->addJSFile("core/js/lib/jquery-migrate.min.js", true);
		...
		$this->addJSLanguage("message.ajaxRequestPending", "message.ajaxDisconnected", "Loading...", "Notifications");
			...
			$this->addJSVar("messageDisplayTime", max(1, (int)C("esoTalk.messageDisplayTime", 20)));
	...
	// Otherwise, set up the master view and render it.
		...
		if ($this->masterView != "json.master" and $this->masterView != "atom.master") {
			...
			$data["pageTitle"] = implode(" - ", $titleParts);
				...
				// Add the forum title, or logo if the configured/classic header uses an image.
				$logo = C("esoTalk.forumLogo", false);
				$title = C("esoTalk.forumTitle");
				if (C("esoTalk.forumHeader", $logo ? "image" : "title") == "image" && !$logo) {
					$logo = "addons/plugins/ClassicCompatibility/resources/icon.svg";
				}
				if ($logo) {
					$logoURL = strpos((string)$logo, "://") !== false ? $logo : getWebPath($logo);
					$data["forumTitle"] = "<img src='".sanitizeHTML($logoURL)."' alt='".sanitizeHTML($title)."'/>";
				} else {
			...
			// Get common menu items (always define keys so views never hit undefined index).
			foreach (array("main", "user", "meta", "statistics") as $menu) {
				$data[$menu."MenuItems"] = "";
			}
...
public function head()
{
	...
	foreach ($this->jsFiles as $files) {
		...
		foreach ($files as $file) {
			...
			if (!$mtime) $mtime = time();
			$head .= "<script src='".getResource($file)."?".$mtime."'></script>\n";
		}
	}
	...
	$head .= "<script>var ET=".json_encode($esoTalkJS, $jsonFlags)."</script>";
