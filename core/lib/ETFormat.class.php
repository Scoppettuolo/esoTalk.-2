...
{
	// Modern URL detection (no fragile TLD whitelist that blanked content on regex failure).
	$this->content = preg_replace_callback(
		'#(^|[\s>(\[])((?:https?://|www\.)[^\s<\]>"\']+)#iu',
		function ($m) {
			$url = rtrim($m[2], ".,;:!?)\"");
			return $m[1].$this->formatLink($url, $url);
		},
		$this->content
	);
	...
	$this->content = preg_replace_callback(
		...
		function ($m) {
			return "<a href='mailto:".$m[1]."' class='link-email'>".$m[1]."</a>";
		},
		$this->content
	);
...
public function linksCallback($matches)
{
	...
	if (!$this->inline and C("esoTalk.format.youtube") and preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $matches[2], $youtube)) {
		...
		$width = 400;
		$height = 225;
		// Modern responsive YouTube iframe (Flash embeds removed from browsers years ago)
		return "<div class='video video-youtube'><iframe width='$width' height='$height' src='https://www.youtube.com/embed/$id' title='YouTube video' frameborder='0' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowfullscreen loading='lazy' referrerpolicy='strict-origin-when-cross-origin'></iframe></div>";
	}
...
public function formatLink($url, $text = null)
{
	...
	$safeText = is_string($text) && $text !== $url ? $text : sanitizeHTML($text);

	// If this is an internal link...
	$baseURL = C("esoTalk.baseURL");
	if ($baseURL && substr($url, 0, strlen($baseURL)) == $baseURL) {
		return "<a href='".$safeUrl."' class='link-internal'>".$safeText."</a>";
	}

	// Otherwise, return an external HTML anchor tag.
	return "<a href='".$safeUrl."' rel='nofollow noopener noreferrer external' target='_blank' class='link-external'>".$safeText." <i class='icon-external-link'></i></a>";
}
...
{
	// Robust quote parser (no /e modifier). Handles [quote], [quote=cite], [quote=id:cite]
	$regexp = '/\[quote(?:=([^\]]+))?\](.*?)\[\/quote\]/is';
	$guard = 0;
	while ($guard++ < 50 && preg_match($regexp, $this->content)) {
		$self = $this;
		$this->content = preg_replace_callback($regexp, function ($m) use ($self) {
			$cite = isset($m[1]) ? $m[1] : "";
			$text = isset($m[2]) ? $m[2] : "";
			return "</p>".$self->makeQuote($text, $cite)."<p>";
		}, $this->content, 1);
	}
	return $this;
}
...
public function mentions()
{
	$this->content = preg_replace_callback(
		...
		function ($m) {
			...
			$url = URL("member/name/".urlencode($name), true);
			return $m[1]."<a href='".$url."' class='link-member'>@".$m[2]."</a>";
		},
		$this->content
	);
