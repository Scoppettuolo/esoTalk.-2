...
protected function getSelect()
{
	...
	$index = ($this->index && !ET::$database->isSQLite()) ? "\nUSE INDEX ($this->index)" : "";
...
protected function getInsert()
{
	...
	if ($onDuplicateKey) {
		$onDuplicateKey = (ET::$database->isSQLite() ? " ON CONFLICT DO UPDATE SET " : " ON DUPLICATE KEY UPDATE ").$onDuplicateKey;
	} else {
		$onDuplicateKey = "";
	}
	return "INSERT INTO $tables ($fields) VALUES $values".$onDuplicateKey;
}
...
protected function getDelete()
{
	...
	if (ET::$database->isSQLite()) {
		// SQLite deletes from one target table at a time. For the multi-table form used by
		// a few cleanup routines, delete the primary table; dependent rows are removed by
		// the corresponding model routines and foreign keys are enabled for SQLite.
		$target = $this->tables[0] ?? "";
		if (preg_match('/^(`[^`]+`)\\s+([A-Za-z_][A-Za-z0-9_]*)$/', $target, $m))
			$target = $m[1]." AS ".$m[2];
		return "DELETE FROM $target ".$this->getWhere();
	}
...
protected function getUnion()
{
	...
	foreach ($selects as &$sql) {
		if (ET::$database->isSQLite()) {
			// Keep component-level ORDER BY/LIMIT clauses inside a derived table;
			// SQLite does not accept a parenthesized SELECT directly as a UNION operand.
			$sql = "\tSELECT * FROM (\n".$sql->get()."\n\t)";
		} else {
			$sql = "\t(".$sql->get().")";
		}
...
public function get()
{
	...
	$query = preg_replace_callback('/(:[A-Za-z0-9_]+)/', function($matches) {
		...
		if (array_key_exists($key, $this->parameters)) {
			return ET::$database->escapeValue($this->parameters[$key][0], $this->parameters[$key][1]);
		}
		return $key;
	}, $query);
