...
{
	// Uploaded or externally hosted avatar takes priority.
	if (!empty($member["memberId"]) and !empty($member["avatarFormat"])) {
		$format = (string)$member["avatarFormat"];
		$url = preg_match("~^https?://~i", $format) ? $format : getWebPath("uploads/avatars/".$member["memberId"].".".$format);
		return "<img src='".sanitizeHTML($url)."' alt='' class='avatar $className' loading='lazy'/>";
	}

	// Optional Gravatar fallback (enabled via plugin or config).
	if (C("plugin.Gravatar.enabled", false) && !empty($member["email"])) {
		$hash = md5(strtolower(trim($member["email"])));
		$size = (int) C("plugin.Gravatar.size", 80);
		if ($size < 16) $size = 16;
		if ($size > 512) $size = 512;
		$rating = C("plugin.Gravatar.rating", "g");
		$default = C("plugin.Gravatar.default", "identicon");
		$src = "https://www.gravatar.com/avatar/".$hash."?s=".$size."&d=".urlencode($default)."&r=".urlencode($rating);
		$dim = min(40, $size);
		return "<img src='$src' alt='' class='avatar $className' width='$dim' height='$dim' loading='lazy'/>";
	}
	...
	if (!empty($member["username"])) {
		$letter = function_exists("mb_substr") ? mb_substr($member["username"], 0, 1) : substr($member["username"], 0, 1);
		$letter = strtoupper($letter);
	}
	return "<span class='avatar $className'>".$letter."</span>";
}
