 ...
 */
public function createActivity($type, $member, $fromMember = null, $data = null, $emailData = null)
{
...
public function getActivity($member, $offset = 0, $limit = 11)
{
	...
	while ($item = $result->nextRow()) {
		...
		$item["data"] = esotalk_unserialize($item["data"]); if (!is_array($item["data"])) $item["data"] = array();
...
public static function updateAvailableNotification($item)
{
	return array(
		...
		!empty($item["data"]["releaseNotes"]) ? $item["data"]["releaseNotes"] : "https://github.com/Scoppettuolo/"
	);
}
