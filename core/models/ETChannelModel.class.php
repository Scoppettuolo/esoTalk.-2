	...
	if (!$this->channels) {
		...
		if (!$channels) {
			...
			foreach ($channels as &$channel) {
				...
				$channel["attributes"] = esotalk_unserialize($channel["attributes"]);
				if (!is_array($channel["attributes"])) $channel["attributes"] = array();
			...
			ET::$cache->store(self::CACHE_KEY, $channels, 600);
		}
 ...
 * @return array An array of channel information indexed by the channel IDs.
 */
public function getByPermission($permission = "view")
{
...
public function addPermissionPredicate(&$sql, $field = "view", $member = false, $table = "c")
{
	...
	// Guests have no member array; use the guest group instead of reading null offsets.
	if (!is_array($member) || empty($member["account"]))
		$groups = ET::groupModel()->getGroupIds(false, false);
	else
