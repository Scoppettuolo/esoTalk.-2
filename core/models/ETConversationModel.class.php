...
public function getEmptyConversation()
{
	...
	if (ET::$session->getValue("membersAllowed")) {
...
public function getMembersAllowed($conversation)
{
	...
	if (!$conversation["conversationId"]) {
		...
		$sessionMembers = (array)ET::$session->getValue("membersAllowed");
	...
	// SQLite does not accept parentheses around each operand of a compound UNION.
 ...
 * 		there was an error.
 */
public function createConversation($data, $membersAllowed = array(), $isDraft = false)
{
	...
	if ($isDraft) {
		...
		$postId = ET::postModel()->createPost($conversationId, ET::$session->userId, $content);
	...
	if (ET::$session) ET::$session->store("lastContentTime", time());
...
public function addReply(&$conversation, $content)
{
	...
	// ETModel::create() accepts one associative array; createPost() is the validated
	// four-argument API used for conversation replies.
	$postId = $postModel->createPost($conversation["conversationId"], ET::$session->userId, $content, $conversation["title"]);
	...
	foreach ($members as $member) {
		ET::activityModel()->createActivity("post", $member, ET::$session->user, $data, $emailData);
	}
...
public function addMember(&$conversation, $member)
{
	...
	// If the conversation doesn't exist, add this member to the session members allowed store.
		...
		$membersAllowed = ET::$session->getValue("membersAllowed", array());
...
protected function privateAddNotification($conversation, $memberIds, $notifyAll = false, $content = null)
{
	...
	$followIds = array();
	foreach ($members as $member) {
		ET::activityModel()->createActivity("privateAdd", $member, ET::$session->user, $data, $emailData);
