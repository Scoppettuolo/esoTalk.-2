...
public function create($values)
{
	...
	ET::activityModel()->createActivity("join", $values);
...
{
	$rows = $this->get(array("m.memberId" => $memberId)); return $rows ? reset($rows) : null;
}
...
public function expand(&$member)
{
	...
	// Unserialize the member's preferences (safe under PHP 7+/8+).
	if (isset($member["preferences"])) {
		if (is_string($member["preferences"])) {
			$prefs = esotalk_unserialize($member["preferences"]);
			$member["preferences"] = is_array($prefs) ? $prefs : array();
		} elseif (!is_array($member["preferences"])) {
			$member["preferences"] = array();
		}
	}
}
...
/**
 * Generate a password hash using PHP's native password_hash (bcrypt).
 * Falls back to phpass only if password_hash is unavailable (very old PHP).
 *
 ...
 * Supports both modern password_hash() hashes and legacy phpass ($P$ / $H$) hashes.
 *
...
{
	// Modern bcrypt / argon hashes
	if (function_exists("password_verify") && (strpos($hash, "$2y$") === 0 || strpos($hash, "$2a$") === 0 || strpos($hash, "$argon") === 0)) {
		return password_verify($password, $hash);
	}
	// Legacy phpass portable hashes
	require_once PATH_LIBRARY."/vendor/phpass/PasswordHash.php";
	$hasher = new PasswordHash(8, false);
...
{
	// A missing member can occur while an anonymous session is being initialized.
			if (!is_array($member) || !isset($member["memberId"]) || !is_object(ET::$session)) return false;
...
{
	// A missing member must never be treated as an authorized target.
			if (!is_array($member) || !isset($member["memberId"], $member["account"]) || !is_object(ET::$session)) return false;
	...
	$canSuspend = is_array(ET::$session->user) && !empty(ET::$session->user["canSuspend"]);
	return
	(
		ET::$session->isAdmin()
		or ($canSuspend and ($member["account"] == ACCOUNT_MEMBER or $member["account"] == ACCOUNT_SUSPENDED))
	)
...
public function setGroups($member, $account, $groups = array())
{
	...
	ET::activityModel()->createActivity("groupChange", $member, ET::$session->user, array("account" => $account, "groups" => $groupNames));
...
public static function getLastActionInfo($time, $action)
{
	...
	$data = esotalk_unserialize($action);
