...
{
	$rows = $this->get(array("p.postId" => $postId)); return $rows ? reset($rows) : null;
}
 ...
 */
public function createPost($conversationId, $memberId, $content, $title = "")
{
	...
	if (C("esoTalk.format.mentions")) {
		...
		if (count($names)) {
			...
			foreach ($members as $member) {
				...
				ET::activityModel()->createActivity("mention", $member, ET::$session->user, $data, $emailData);
			}
		}
