...
public $orderReverse = false;
	...
	/** @var array Regex matches from gambit conditions */
 ...
 * $condition as a callable (preferred) or legacy eval string.
 *
 * @param callable|string $condition callable($term, $sender): bool, or legacy expression string.
 * 		Example: function ($term) { return $term === "sticky"; }
...
{
	// $condition: callable($term, $sender): bool  OR legacy string for eval() (plugins)
	self::$gambits[] = array($condition, $function);
}

/**
 * Evaluate a gambit condition without requiring string eval when a callable is provided.
 *
 * @param mixed $condition callable or legacy PHP expression string
 * @param string $term Lowercase search term
 * @return bool
 */
protected function matchGambitCondition($condition, $term)
{
	if (is_callable($condition)) {
		return (bool)call_user_func($condition, $term, $this);
	}
	// Legacy string conditions (plugins may still register these)
...
public function isFlooding()
{
	...
	$searches = ET::$session->getValue("searches");
	if (!empty($searches)) {
...
public function getConversationIDs($channelIDs = array(), $searchString = "", $orderBySticky = false)
{
	...
	foreach ($terms as $term) {
		...
		$term = trim($term);
		if ($term === "") continue;
		$negate = (isset($term[0]) && $term[0] === "!");
		if ($negate) $term = trim($term, "! ");
		if ($term === "") continue;
		...
		if (isset($term[0]) && $term[0] === "#") {
			...
			foreach (self::$gambits as $gambit) {
				...
				if ($this->matchGambitCondition($condition, $term)) {
...
public function removeGambit($searchString, $condition)
{
	...
	foreach ($terms as $k => $term) {
		...
		if (isset($term[0]) && $term[0] === "#") {
			...
			// Match gambit condition (callable preferred; legacy eval string still supported).
			if ($this->matchGambitCondition($condition, $term)) {
				unset($terms[$k]);
				continue;
			}
...
// Add default gambits.
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.starred")); }, array("ETSearchModel", "gambitStarred"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.ignored")); }, array("ETSearchModel", "gambitIgnored"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.draft")); }, array("ETSearchModel", "gambitDraft"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.private")); }, array("ETSearchModel", "gambitPrivate"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.sticky")); }, array("ETSearchModel", "gambitSticky"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.locked")); }, array("ETSearchModel", "gambitLocked"));
ETSearchModel::addGambit(function ($term) { return strpos($term, strtolower(T("gambit.author:"))) === 0; }, array("ETSearchModel", "gambitAuthor"));
ETSearchModel::addGambit(function ($term) { return strpos($term, strtolower(T("gambit.contributor:"))) === 0; }, array("ETSearchModel", "gambitContributor"));
ETSearchModel::addGambit(function ($term, $sender) { return (bool)preg_match(T("gambit.gambitActive"), $term, $sender->matches); }, array("ETSearchModel", "gambitActive"));
ETSearchModel::addGambit(function ($term, $sender) { return (bool)preg_match(T("gambit.gambitHasNReplies"), $term, $sender->matches); }, array("ETSearchModel", "gambitHasNReplies"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.order by replies")); }, array("ETSearchModel", "gambitOrderByReplies"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.order by newest")); }, array("ETSearchModel", "gambitOrderByNewest"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.unread")); }, array("ETSearchModel", "gambitUnread"));
ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.reverse")); }, array("ETSearchModel", "gambitReverse"));
ETSearchModel::addGambit(function ($term) { return strpos($term, strtolower(T("gambit.limit:"))) === 0; }, array("ETSearchModel", "gambitLimit"));

if (!C("esoTalk.search.disableRandomGambit"))
	ETSearchModel::addGambit(function ($term) { return $term === strtolower(T("gambit.random")); }, array("ETSearchModel", "gambitRandom"));
