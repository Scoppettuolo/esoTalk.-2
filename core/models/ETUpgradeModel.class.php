...
public function checkForUpdates()
{
	...
	if (($package = ET::checkForUpdates()) and $package["version"] != $info["version"]) {
		...
		// Create a notification for the current user (if logged in).
		if (ET::$session->user) {
			ET::activityModel()->createActivity("updateAvailable", ET::$session->user, null, $package);
		}
