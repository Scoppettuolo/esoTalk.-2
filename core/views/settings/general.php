...
<script>
$(function() {
	var updateAvatarFields = function() {
		var source = $("#settings-general input[name=avatarSource]:checked").val();
		$("#settings-general input[name=avatar]").closest("label").toggle(source == "upload");
		$("#settings-general input[name=avatarURL]").closest("label").toggle(source == "url");
	};
	$("#settings-general input[name=avatarSource]").change(updateAvatarFields);
	updateAvatarFields();
});
</script>
